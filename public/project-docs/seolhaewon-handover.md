...
| favicon 연결     | `index.html`의 `<link rel="icon" href="/favicon.png" />` | 브라우저 탭 아이콘 경로                         |
...
1. zip으로 프로젝트를 전달받은 경우 `.env` 파일을 생성하고,
   파일 내부에 VITE_API_BASE_URL=[운영주소]/hsw 를 작성해야 합니다.
   서버를 로컬로 켜서 테스트 시 서버 주소를 넣어서 테스트할 수 있습니다/

2. 링크 열기는 현재 `value`만 있으면 열리도록 되어 있습니다.
   운영 반영 시에는 https 검사 함수를 켜는 것을 검토하고, 요구사항에 따라 켜지 않아도 무관할 수 있습니다.

3. 스타일링 수정 시 Tailwind CSS 문법은 https://tailwindcss.com/ 에서 CSS 속성을 검색하면 확인할 수 있습니다. 필요한 class를 `className`에 추가하거나 변경하면 반영됩니다.

4. 기본적인 문구 수정은 `Ctrl + F`로 텍스트를 검색해서 변경하고, 로컬에서 확인하는 방식으로 처리하는게 가장 간단합니다. 이외에는 개발자도구에서 className 값을 검사하거나 인수인계서에서 파일을 찾아 변경하시면 됩니다.
