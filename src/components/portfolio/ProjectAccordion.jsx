...
const ProjectCaseStudies = ({ items }) => {
  ...
  return (
    <section className="rounded-md border border-slate-200 bg-slate-50 p-4">
      <p className="text-sm font-black text-slate-950">문제 해결 사례</p>
        ...
        {items.map((item) => (
          ...
          >
            ...
            <dl className="mt-4 grid gap-3 lg:grid-cols-[1fr_auto_1fr_auto_1fr] lg:items-start">
              <div>
                <dt className="text-xs font-black text-sky-700">문제</dt>
                <dd className="mt-1 whitespace-pre-line break-words text-sm font-medium leading-6 text-slate-700">
                  {item.problem}
                </dd>
              </div>
              <FlowArrow />
              <div>
                <dt className="text-xs font-black text-sky-700">해결</dt>
                <dd className="mt-1 whitespace-pre-line break-words text-sm font-medium leading-6 text-slate-700">
                  {item.approach}
                </dd>
              </div>
              <FlowArrow />
              <div>
                <dt className="text-xs font-black text-sky-700">결과</dt>
                <dd className="mt-1 whitespace-pre-line break-words text-sm font-medium leading-6 text-slate-700">
                  {item.result}
                </dd>
              </div>
  ...
  return (
    <>
      <article
        ...
        className="group min-w-0 scroll-mt-6 overflow-hidden rounded-lg border border-slate-200 bg-white transition hover:border-sky-300 hover:shadow-[0_10px_30px_rgba(15,23,42,0.06)]"
      >
        <button
          type="button"
        ...
        >
          <div className="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-start">
            ...
            <div className="flex min-w-0 flex-1 flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              ...
              <span className="inline-flex shrink-0 items-center justify-center rounded border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-black text-slate-700 transition group-hover:border-sky-500 group-hover:bg-sky-50 group-hover:text-sky-700">
                상세 보기
              </span>
            </div>
          </div>
      ...
      {isOpen && (
        ...
        >
          <article
            className="flex h-[90dvh] w-full min-w-0 flex-col overflow-hidden bg-white shadow-2xl sm:w-[80vw] sm:rounded-xl"
          ...
          >
            <header className="shrink-0 border-b border-slate-200 bg-white px-4 py-4 sm:px-6">
              <div className="flex items-start justify-between gap-4">
                ...
                <button
                  type="button"
                  onClick={onClose}
                  className="shrink-0 rounded border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-black text-slate-700 transition hover:border-sky-500 hover:bg-sky-50 hover:text-sky-700"
                >
                  닫기
                </button>
              </div>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-5 sm:px-6 sm:py-6">
              <div className="mx-auto flex max-w-6xl flex-col gap-5">
                <section className="rounded-md border border-slate-200 bg-slate-50 p-4">
                  <p className="text-[0.95rem] font-semibold leading-7 text-slate-800">
                    {project.summary}
                  </p>
                  <div className="mt-4">
                    <TagList items={project.tech} />
